...
build/define dependent variable
municipality election results

the four elections in 2019 and for first-differenced specifications the corresponding counterparts in 2015.\footnote{In our analysis, we exclude areas that were subject to territory  reforms between the elections 2015-2019. This excludes X\% of the observations.}


% data sources
For the elections for the European Parliament, we use data provided by the Federal Statistical Office and the statistical offices of the Länder. For the state elections in Brandenburg, Saxony, and Thuringia we draw on data from the State Returning Officers (\textit{Landeswahlleiter}) and the statistical offices of the Länder.


define dependent variable as the vote share: the number of votes for the Greens divided by total valid votes cast.
...
The Federal Statistical Office and the statistical offices of the Länder provide a database (\textit{Regionaldatenbank}) of detailed statistics by different subject areas at very granular spatial levels. We draw on this database for various purposes. 


weights and independent variable (ratio):Population Figures
controls: income, unemployment, demographics
